...
resource "null_resource" "ansible_pull" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "ansible-pull -i localhost, -U https://github.com/tejadevsecops/roboshop-ansible-new roboshop.yaml -e env=${var.env} -e component=${var.component_name} -e vault_token=${var.vault_token}"
    ]
  }
